...
- ⚡ **本地优先 OCR 异步预热**：基于独立 Python OCR Worker (PaddleOCR PP-OCRv5)，完全本地运行，不依赖云端；HTTP 服务会快速启动，模型首次加载时间取决于设备与模型缓存。
- 🔍 **原图-文本双栏联动校对**：
  - 左侧原图查看器支持平移、缩放、双击适应、1:1 原图、磨砂玻璃浮动 HUD。
  - 自动标出低置信度文字区域（默认 `< 0.75` 黄色边框高亮）。
  - 右侧文本编辑器实时联动，支持点击跳转定位与「上一处 / 下一处低置信度」一键跳转。
- ✍️ **辅助过滤印刷体与相机水印**：基于像素形态学（笔画宽度方差与轴向梯度分布）进行启发式分类，支持实时开关；过滤操作不会覆盖已经人工校对的文本。
...
打开终端（PowerShell 或 CMD），进入项目根目录安装 Python 依赖（锁定的 PP-OCRv5 直接依赖）：
```powershell
